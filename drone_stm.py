...
class Drone:
    def __init__(self):
        ...
        self.mqtt_host = os.getenv('MQTT_SERVER', '192.168.10.134')
        ...
        self.client.subscribe("ttm4115/team03/drone")
    ...
    def _animation_loop(self):
        """Runs in the background and updates the SenseHat matrix based on current_animation"""
    ...
    def publish_status(self, status):
        ...
        self.client.publish("ttm4115/team03/server", json.dumps(payload))

    def notify_charging_station(self, action):
        payload = {"action": action, "drone_id": self.drone_id}
        if self.current_order_id is not None:
            payload["id"] = self.current_order_id
        self.client.publish("ttm4115/team03/charging", json.dumps(payload))
...
state_wait_presence = {
    'name': 'Wait_Presence',
    'entry': 'hover; start_timer("timout", 30000)',
    'exit': 'stop_timer("timout")'
}
...
t12 = {'trigger': 'timout', 'source': 'Wait_Presence', 'target': 'Abort_Return'}
